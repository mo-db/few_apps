...
static float volume_slider_val = 0.0;
...
int main(int argc, char **argv)
{
	...
	SDL_Rect rect_volume_knob = { rect_volume.x, rect_volume.y , 10, rect_volume.h};
	...
	while (keep_going) {
		...
		if (SDL_GetQueuedAudioSize(audio_device) < 8192) {
			...
			if (bytes_remaining > 0 ) {
				...
				const int num_samples = (new_bytes / sizeof(float)); // bc we use audio_f32
