...
static float volume_slider_val = 1.0;
static float balance_slider_val = 0.5;
...
static int open_new_audio_file(const char *file_name) 
{
	...
	if (SDL_AudioStreamPut(stream, wavbuf, wavlen) != 0) {
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "couldn't put data to audio stream", SDL_GetError(), window);
		goto cleanup;
	}
	if (SDL_AudioStreamFlush(stream) == -1) {
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "flush failed", SDL_GetError(), window);
		goto cleanup;
	}
...
int main(int argc, char **argv)
{
	...
	SDL_Rect rect_pause = { 50, 250, 200, 100 };
	int rect1_color = 0;
	SDL_Rect rect_rewind = { 500, 50, 200, 100};
	int rect2_color = 0;
	SDL_Rect rect_volume = { (640-500) / 2, 450, 500, 20};
	SDL_Rect rect_balance = { (640-500) / 2, 300, 500, 20};
	SDL_Rect rect_volume_knob = { rect_volume.x + rect_volume.w - rect_volume_knob.w, rect_volume.y , 10, rect_volume.h};
	SDL_Rect rect_balance_knob = { rect_balance.x + (rect_balance.w / 2) - rect_balance_knob.w, rect_balance.y , 10, rect_balance.h};
	...
	while (keep_going) {
		...
		while (SDL_PollEvent(&event)) {
			switch(event.type) {
				...
				case SDL_MOUSEBUTTONDOWN: {
					...
					if (SDL_PointInRect(&pt, &rect_rewind)) {
						...
						if (SDL_AudioStreamPut(stream, wavbuf, wavlen) == -1) {
							SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "couldnt put audio data to stream", SDL_GetError(), window);
							stop_audio();
				...
				case SDL_MOUSEMOTION: {
					...
					// state is a bitmap -> bitweise and to check state NOTE: ?
					if (SDL_PointInRect(&pt, &rect_volume) && (event.motion.state & SDL_BUTTON_LMASK)) {
						const float fx = (float)(pt.x - rect_volume.x);
						volume_slider_val = (fx / (float)(rect_volume.w));
						printf("sliding at %d/%d volume: %d\n", pt.x, pt.y, (int)SDL_round(volume_slider_val * 100.0));
						rect_volume_knob.x = pt.x - (rect_volume_knob.w / 2);
					}
					if (SDL_PointInRect(&pt, &rect_balance) && (event.motion.state & SDL_BUTTON_LMASK)) {
						const float fx = (float)(pt.x - rect_balance.x);
						balance_slider_val = (fx / (float)(rect_balance.w));
						printf("sliding at %d/%d balance: %d\n", pt.x, pt.y, (int)SDL_round(balance_slider_val * 100.0));
		...
		if (SDL_GetQueuedAudioSize(audio_device) < 8192) {
			...
			if (bytes_remaining > 0 ) {
				...
				int num_converted_bytes = SDL_AudioStreamGet(stream, converted_buffer, new_bytes); 
				// >> draw bytes from stream into new buffer
				if (num_converted_bytes > 0) {
					// we no longer talk in bytes, but in samples (not in sample-frames -> two samples for stereo for example)
					const int num_samples = (num_converted_bytes / sizeof(float)); // bc we use audio_f32
					float *samples = (float*)converted_buffer;

					// change the volume of audio playing
					if (volume_slider_val != 1.0) {
						for (int i = 0; i < num_samples; i++) {
							samples[i] *= volume_slider_val; // NOTE: so we need floatingpoint audio to do this!?
						}
					}
					
					// change the balance of audio playing
					// we process two samples at a time, samples are saved L then R and so on
					// still not fully right, need 2 slider vals i think
			
					if (balance_slider_val > 0.5) {
						for (int i = 0; i < num_samples; i += 2) {
							samples[i] *= (1.0 - balance_slider_val);
						}
					} else if (balance_slider_val < 0.5) {
						for (int i = 0; i < num_samples; i += 2) {
							samples[i+1] *= balance_slider_val;
						}
					}
		...
		SDL_RenderFillRect(renderer, &rect_volume);
		SDL_RenderFillRect(renderer, &rect_balance);

		SDL_SetRenderDrawColor(renderer, 0, 0, 255, 255);
		SDL_RenderFillRect(renderer, &rect_volume_knob);
		SDL_RenderFillRect(renderer, &rect_balance_knob);
